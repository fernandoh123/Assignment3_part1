...
![alt text](http://118.138.234.73:8787/files/Assignment3_part1/Hist_Mean.png)
...
head(res)
```
My sequence has only 1 hit with 100% similarity and no mismatches. The name of my sequence is AAC75595.
...
mygene_mutation <- mutator(myseq=mygene,400)
res <- myblastn_tab(myseq = mygene_mutation, db = "mygene.fa")
res
```
#### Test with 310 mismatches
```{r, chunk 39, echo=TRUE}
mygene_mutation <- mutator(myseq=mygene,500)
res <- myblastn_tab(myseq = mygene_mutation, db = "mygene.fa")
res
```

### Q6. Chart to show how mutation affects BLAST performance
![alt text](http://118.138.234.73:8787/files/Assignment3_part1/BLAST%20plot.jpg)
